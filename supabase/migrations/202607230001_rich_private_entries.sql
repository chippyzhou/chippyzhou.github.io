...
create unique index if not exists guestbook_messages_invite_request_key
  on public.guestbook_messages (invite_id, client_request_id)
...
create or replace function public.post_guestbook_message_v2(
  session_token text,
  message_body text,
  request_id uuid
)
...
declare
  invite public.visitor_invites;
  saved_message public.guestbook_messages;
begin
  select * into invite from public.valid_visitor(session_token);
  if invite.id is null then raise exception 'Your invitation is no longer active.'; end if;
  ...
  if saved_message.id is null then
    insert into public.guestbook_messages (invite_id, body, client_request_id)
    values (invite.id, trim(message_body), request_id)
...
grant execute on function public.post_guestbook_message_v2(text, text, uuid) to anon, authenticated;
